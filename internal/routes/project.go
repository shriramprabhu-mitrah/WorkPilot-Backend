...
func ProjectRoutes(deps models.Config, api *gin.RouterGroup) {
	...
	prj := api.Group("/project")
	{
		...
		prj.POST("/create", middleware.ValidateJWT(), projectHandler.CreateProject)
		...
		prj.POST("/add-members", middleware.ValidateJWT(), projectHandler.CreateProjectMember)
		...
		prj.DELETE("/:project_id", middleware.ValidateJWT(), projectHandler.Deleteproject)
