...
type UserStoryAttachment struct {
	ID               uuid.UUID `json:"id" gorm:"type:uuid;primaryKey"`
	UserStoryID      uuid.UUID `json:"user_story_id" gorm:"type:uuid;not null;index"`
...
func (a *UserStoryAttachment) BeforeCreate(tx *gorm.DB) (err error) {
	if a.ID == uuid.Nil {
		a.ID, err = uuid.NewV7()
		if err != nil {
			return err
